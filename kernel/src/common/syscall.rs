...
pub unsafe extern "C" fn syscall_dispatch(num: u64, arg1: u64, arg2: u64, _arg3: u64, _arg4: u64, _arg5: u64) -> u64 {
    match num {
        SYS_WRITE => {
            ...
            for i in 0..len {
                drivers::serial::serial_putchar(*str.add(i) as char);
            }
            len as u64
        }
        SYS_READ => 0,
        ...
        SYS_CLOSE => 0,
        SYS_EXIT => {
            crate::common::nebula::kernel_panic("Process exited");
            0
        }
        SYS_GETPID => {
            let proc = crate::common::process::process_get_current();
            if !proc.is_null() {
                (*proc).pid as u64
            } else {
                0
        ...
        SYS_YIELD => {
            crate::common::scheduler::scheduler_tick();
            0
        }
        _ => -1i64 as u64,
    }
