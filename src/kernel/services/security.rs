...
impl SecurityService {
    ...
    pub fn delete_user(&mut self, uid: u32) -> Result<(), &'static str> {
        ...
        if let Some(_user) = self.users.remove(&uid) {
    ...
    pub fn delete_group(&mut self, gid: u32) -> Result<(), &'static str> {
        ...
        if let Some(_group) = self.groups.remove(&gid) {
    ...
    pub fn check_permission(&self, uid: u32, _permission: Permission) -> bool {
...
/// Global security service instance - lazily initialized
static SECURITY_INITIALIZED: AtomicBool = AtomicBool::new(false);
static mut SECURITY_SERVICE_DATA: Option<crate::sync::Spinlock<SecurityService>> = None;

pub fn get_security_service() -> &'static crate::sync::Spinlock<SecurityService> {
    if !SECURITY_INITIALIZED.load(Ordering::SeqCst) {
        unsafe {
            SECURITY_SERVICE_DATA = Some(crate::sync::Spinlock::new(SecurityService::new()));
            SECURITY_INITIALIZED.store(true, Ordering::SeqCst);
        }
    }
    unsafe { SECURITY_SERVICE_DATA.as_ref().unwrap() }
}

/// Initialize the security service
pub fn init() {
    // Security service is initialized automatically
    let _ = get_security_service();
}
