...
static mut PROCESS_TABLE: [Process; MAX_PROCESSES] = [Process { pid: 0, state: 0, context: unsafe { core::mem::zeroed() }, stack_base: 0, stack_size: 0, entry_point: core::ptr::null_mut(), fd_table: [-1; 8] }; MAX_PROCESSES];
static mut NEXT_PID: u32 = 1;
static mut CURRENT_PROCESS: *mut Process = core::ptr::null_mut();

pub unsafe fn process_init() {
    for i in 0..MAX_PROCESSES {
        (*PROCESS_TABLE.as_mut_ptr().add(i)).pid = 0;
        (*PROCESS_TABLE.as_mut_ptr().add(i)).state = PROC_ZOMBIE;
    }
...
pub unsafe fn process_create(entry: *mut u8, stack_size: u32) -> i32 {
    for i in 0..MAX_PROCESSES {
        let proc = &mut *PROCESS_TABLE.as_mut_ptr().add(i);
...
pub unsafe fn process_set_current(pid: u32) {
    for i in 0..MAX_PROCESSES {
        if (*PROCESS_TABLE.as_ptr().add(i)).pid == pid {
            CURRENT_PROCESS = PROCESS_TABLE.as_mut_ptr().add(i);
            return;
        }
    }
}
...
pub unsafe fn process_destroy(pid: u32) {
    for i in 0..MAX_PROCESSES {
        if (*PROCESS_TABLE.as_ptr().add(i)).pid == pid {
            (*PROCESS_TABLE.as_mut_ptr().add(i)).state = PROC_ZOMBIE;
            if CURRENT_PROCESS == PROCESS_TABLE.as_ptr().add(i) as *mut Process {
                CURRENT_PROCESS = core::ptr::null_mut();
            }
            return;
        }
