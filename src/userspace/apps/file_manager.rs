    ...
    pub fn refresh_files(&mut self) {
        ...
        if let Some(ref mut fs) = self.fs {
            ...
            for file in &test_files {
                let _ = FileSystemOps::create_file(fs, 2, file);
            }

            let test_dirs = ["Documents", "Downloads", "Pictures"];
            for dir in &test_dirs {
                let _ = FileSystemOps::create_dir(fs, 2, dir);
        ...
        match c {
            ...
            '\n' => {
                if let Some(selected) = self.selected_file {
                    ...
                    if filename == ".." {
                        ...
                        if let Some(ref mut fs) = self.fs {
                            match FileSystemOps::lookup(fs, 2, filename) {
                                Ok(_) => {
            ...
            'n' => {
                if let Some(ref mut fs) = self.fs {
                    ...
                    match FileSystemOps::create_file(fs, 2, new_filename) {
                        Ok(_) => {
                            self.refresh_files();
                        }
                        Err(_) => {}
            ...
            'd' => {
                if let Some(ref mut fs) = self.fs {
                    ...
                    match FileSystemOps::create_dir(fs, 2, new_dirname) {
                        Ok(_) => {
                            self.refresh_files();
                        }
                        Err(_) => {}
