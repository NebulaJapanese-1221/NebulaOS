// x86 (32-bit) CPU exception handlers
...
pub unsafe fn init() {
    ...
    idt::set_gate(0, divide_by_zero_asm as *const () as u32, 0x08, 0x8E);
    idt::set_gate(6, invalid_opcode_asm as *const () as u32, 0x08, 0x8E);
    idt::set_gate(13, gpf_asm as *const () as u32, 0x08, 0x8E);
    idt::set_gate(14, page_fault_asm as *const () as u32, 0x08, 0x8E);
}

#[no_mangle]
pub extern "C" fn divide_by_zero_handler(regs: &crate::syscalls::SyscallRegisters) {
    let eip = regs.eip;
    crate::serial_println!("DIVIDE BY ZERO at EIP: 0x{:08x} (User: {})", eip, regs.is_user());
...
pub extern "C" fn invalid_opcode_handler(regs: &crate::syscalls::SyscallRegisters) {
    let eip = regs.eip;
    crate::serial_println!("INVALID OPCODE at EIP: 0x{:08x} (User: {})", eip, regs.is_user());
...
pub extern "C" fn gpf_handler(regs: &crate::syscalls::SyscallRegisters) {
    let eip = regs.eip;
    crate::serial_println!("GPF at EIP: 0x{:08x} (User: {})", eip, regs.is_user());
...
pub extern "C" fn page_fault_handler(regs: &crate::syscalls::SyscallRegisters) {
    let eip = regs.eip;
    crate::serial_println!("PAGE FAULT at EIP: 0x{:08x} (User: {})", eip, regs.is_user());
