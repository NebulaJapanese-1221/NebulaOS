...
pub struct PowerManagementService {
    ...
    restart_requested: AtomicBool,  // Whether restart was requested
}
...
/// Global power management service instance - lazily initialized
static POWER_INITIALIZED: AtomicBool = AtomicBool::new(false);
static mut POWER_SERVICE_DATA: Option<crate::sync::Spinlock<PowerManagementService>> = None;

pub fn get_power_service() -> &'static crate::sync::Spinlock<PowerManagementService> {
    if !POWER_INITIALIZED.load(Ordering::SeqCst) {
        unsafe {
            POWER_SERVICE_DATA = Some(crate::sync::Spinlock::new(PowerManagementService::new()));
            POWER_INITIALIZED.store(true, Ordering::SeqCst);
        }
    }
    unsafe { POWER_SERVICE_DATA.as_ref().unwrap() }
}

/// Initialize the power management service
pub fn init() {
    // Power management service is initialized automatically
    let _ = get_power_service();
}
