...
impl Socket {
    ...
    pub fn listen(&mut self, _backlog: i32) -> Result<(), &'static str> {
...
/// Global network service instance - lazily initialized
static NETWORK_INITIALIZED: AtomicBool = AtomicBool::new(false);
static mut NETWORK_SERVICE_DATA: Option<crate::sync::Spinlock<NetworkService>> = None;

pub fn get_network_service() -> &'static crate::sync::Spinlock<NetworkService> {
    if !NETWORK_INITIALIZED.load(Ordering::SeqCst) {
        unsafe {
            NETWORK_SERVICE_DATA = Some(crate::sync::Spinlock::new(NetworkService::new()));
            NETWORK_INITIALIZED.store(true, Ordering::SeqCst);
        }
    }
    unsafe { NETWORK_SERVICE_DATA.as_ref().unwrap() }
}

/// Initialize the network service
pub fn init() {
    let mut service = get_network_service().lock();
