...
impl VFS {
    ...
    pub fn lookup(&self, path: &str) -> Result<VFSNode, &'static str> {
        ...
        let mut current_inode = self.root.inode;
        ...
        for component in components {
            ...
            if component == ".." {
                ...
                current_inode = self.root.inode;
                continue;
            }
            
            // Find the component in the current directory
            let entries = self.readdir(current_inode)?;
        ...
        // Find the final node
        if current_inode == self.root.inode {
            return Ok(self.root.clone());
        }
        for mounted_fs in &self.mounted_fs {
            match mounted_fs.fs.stat(current_inode) {
...
impl FileSystem for NebulaFS {
    ...
    fn unmount(&self) -> Result<(), &'static str> {
        // Unsupported: &self is immutable, but unmount needs &mut self
        // NebulaFS::unmount takes &mut self, so we can't call it from &self
        Ok(())
    }
