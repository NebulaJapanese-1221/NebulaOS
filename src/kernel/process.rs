...
// In a single-CPU environment, raw pointers in Thread are safe to send between contexts
// as long as we don't actually move threads between CPUs
...
impl ProcessManager {
    ...
    pub fn send_signal_to_group(&mut self, pgid: usize, signal: Signal) {
        let pids: Vec<usize> = self.process_groups.get(&pgid)
            .map(|p| p.clone())
            .unwrap_or_default();
        for &pid in &pids {
