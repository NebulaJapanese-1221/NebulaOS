use crate::common::stdint::{u16, u32, u8};
...
pub unsafe fn scroll_up() {
    ...
    for x in 0..VGA_WIDTH {
        set_char(x, (VGA_HEIGHT - 1) as u8, b' ', CURRENT_COLOR);
    }
    if CURSOR_Y > 0 {
        CURSOR_Y -= 1;
    }
...
pub unsafe fn putchar(c: char) {
    match c {
        ...
        _ => {
            set_char(CURSOR_X, CURSOR_Y, c, CURRENT_COLOR);
            CURSOR_X += 1;
            if CURSOR_X >= VGA_WIDTH as u8 {
                CURSOR_X = 0;
                CURSOR_Y += 1;
