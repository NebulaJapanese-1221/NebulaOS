...
const LOGOUT_TEXT_COLOR: u32 = 0x00FF6666;
...
pub fn draw(fb: &mut Framebuffer, y_start: u32) {
    ...
    fb.draw_rect(item_area_x_start, current_y as usize, item_area_x_end, MENU_ITEM_HEIGHT as usize, MENU_ITEM_BG_COLOR);
    draw_string(fb, (MENU_ITEM_PADDING + 5) as usize, (current_y + 8) as usize, "System Settings", MENU_TEXT_COLOR);
    current_y += MENU_ITEM_HEIGHT + MENU_ITEM_PADDING;

    // System Monitor Entry
    fb.draw_rect(item_area_x_start, current_y as usize, item_area_x_end, MENU_ITEM_HEIGHT as usize, MENU_ITEM_BG_COLOR);
    draw_string(fb, (MENU_ITEM_PADDING + 5) as usize, (current_y + 8) as usize, "System Monitor", MENU_TEXT_COLOR);
    current_y += MENU_ITEM_HEIGHT + MENU_ITEM_PADDING;

    // Logout Entry (always at bottom, with red tint)
    fb.draw_rect(item_area_x_start, current_y as usize, item_area_x_end, MENU_ITEM_HEIGHT as usize, 0x00442222);
    draw_string(fb, (MENU_ITEM_PADDING + 5) as usize, (current_y + 8) as usize, "Logout", LOGOUT_TEXT_COLOR);
}

pub fn handle_click(mx: i32, my: i32, height: i32, wm: &mut WindowManager, start_menu_open: &mut bool, logged_in: &mut bool) {
    ...
    if mx >= item_area_x_start && mx <= item_area_x_end &&
        ...
        *start_menu_open = false;
        wm.windows.clear(); // Close all windows on logout
        return;
    }
