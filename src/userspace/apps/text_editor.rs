...
impl TextEditorApp {
    pub fn draw(fb: &mut Framebuffer, bounds: Rect, state: &TextEditorState, _is_focused: bool) {
        ...
        for (i, line) in state.content.iter().skip(start_line).take(end_line - start_line).enumerate() {
            // Draw line number
