...
pub unsafe extern "C" fn shell_keyboard_callback(_scancode: u8, _modifiers: u8, _pressed: u8) {
}

unsafe fn process_scancode(_scancode: u8) {
    if _scancode & 0x80 != 0 {
        return;
    }
    match _scancode {
        ...
        _ => {
            let _modifiers = 0u8;
            let _c = 0u8;
            if _c >= 32 && _c <= 126 && COMMAND_POS < SHELL_BUFFER_SIZE - 1 {
                COMMAND_BUFFER[COMMAND_POS] = _c as u8;
                COMMAND_POS += 1;
                COMMAND_BUFFER[COMMAND_POS] = 0;
                vga::putchar(_c as char);
            }
        }
    }
}
...
unsafe fn parse_command(input: &[u8], argv: &mut [*mut u8]) -> i32 {
    let mut argc: i32 = 0;
    let mut ptr = input.as_ptr() as *mut u8;
    let mut start = ptr;
    while *ptr == b' ' || *ptr == b'\t' {
        ptr = ptr.add(1);
        start = ptr;
    }
    while *ptr != 0 && argc < SHELL_MAX_ARGS as i32 {
        if *ptr == b' ' || *ptr == b'\t' {
            if start != ptr {
                *ptr = 0;
                argv[argc as usize] = start as *mut u8;
                argc += 1;
            }
            start = ptr.add(1);
        }
    ...
    if *start != 0 && argc < SHELL_MAX_ARGS as i32 {
        argv[argc as usize] = start as *mut u8;
        argc += 1;
    }
    argv[argc as usize] = ptr::null_mut::<u8>();
    argc
}
...
unsafe extern "C" fn cmd_help(_argc: i32, _argv: *mut *mut u8) {
    vga::puts(b"Available commands: help clear reboot meminfo echo color version ls cat ps exec kill touch mkdir\n\0" as *const u8 as *const u8);
}

unsafe extern "C" fn cmd_clear(_argc: i32, _argv: *mut *mut u8) {
    vga::clear();
    vga::puts(b"NebulaOS Shell\n\0" as *const u8 as *const u8);
    vga::puts(b"> \0" as *const u8 as *const u8);
}

unsafe extern "C" fn cmd_reboot(_argc: i32, _argv: *mut *mut u8) {
    vga::puts(b"Rebooting...\n\0" as *const u8 as *const u8);
}

unsafe extern "C" fn cmd_meminfo(_argc: i32, _argv: *mut *mut u8) {
...
unsafe extern "C" fn cmd_echo(_argc: i32, argv: *mut *mut u8) {
    if _argc < 2 {
...
unsafe extern "C" fn cmd_color(_argc: i32, _argv: *mut *mut u8) {
    if _argc < 2 {
...
unsafe extern "C" fn cmd_version(_argc: i32, _argv: *mut *mut u8) {
    vga::puts(b"NebulaOS Version: 0.0.1\n\0" as *const u8 as *const u8);
}

unsafe extern "C" fn cmd_ls(_argc: i32, _argv: *mut *mut u8) {
    vga::puts(b"ls: stub\n\0" as *const u8 as *const u8);
}

unsafe extern "C" fn cmd_cat(_argc: i32, _argv: *mut *mut u8) {
    vga::puts(b"cat: stub\n\0" as *const u8 as *const u8);
}

unsafe extern "C" fn cmd_ps(_argc: i32, _argv: *mut *mut u8) {
    vga::puts(b"PID  STATE     ENTRY\n\0" as *const u8 as *const u8);
}

unsafe extern "C" fn cmd_exec(_argc: i32, _argv: *mut *mut u8) {
    vga::puts(b"exec: stub\n\0" as *const u8 as *const u8);
}

unsafe extern "C" fn cmd_kill(_argc: i32, _argv: *mut *mut u8) {
    vga::puts(b"kill: stub\n\0" as *const u8 as *const u8);
}

unsafe extern "C" fn cmd_touch(_argc: i32, _argv: *mut *mut u8) {
    vga::puts(b"touch: not implemented\n\0" as *const u8 as *const u8);
}

unsafe extern "C" fn cmd_mkdir(_argc: i32, _argv: *mut *mut u8) {
