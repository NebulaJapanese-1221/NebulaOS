use crate::stdint::*;
use crate::process::{self, Process, PROC_READY};

pub const SCHEDULER_TIME_SLICE: u32 = 10;

static mut SCHEDULER_QUEUE: [u32; process::MAX_PROCESSES as usize] = [0; process::MAX_PROCESSES as usize];
...
pub unsafe fn scheduler_add(pid: u32) {
    if QUEUE_COUNT >= process::MAX_PROCESSES {
        return;
    }
    SCHEDULER_QUEUE[QUEUE_TAIL] = pid;
    QUEUE_TAIL = (QUEUE_TAIL + 1) % process::MAX_PROCESSES as usize;
    QUEUE_COUNT += 1;
}

pub unsafe fn scheduler_remove(pid: u32) {
    for i in 0..QUEUE_COUNT {
        let idx = (QUEUE_HEAD + i) % process::MAX_PROCESSES as usize;
        if SCHEDULER_QUEUE[idx] == pid {
            for j in i..QUEUE_COUNT - 1 {
                let cur = (QUEUE_HEAD + j) % process::MAX_PROCESSES as usize;
                let nxt = (QUEUE_HEAD + j + 1) % process::MAX_PROCESSES as usize;
                SCHEDULER_QUEUE[cur] = SCHEDULER_QUEUE[nxt];
            }
            QUEUE_TAIL = (QUEUE_TAIL - 1 + process::MAX_PROCESSES as usize) % process::MAX_PROCESSES as usize;
            QUEUE_COUNT -= 1;
            return;
        }
    }
...
pub unsafe fn scheduler_tick() {
    ...
    QUEUE_HEAD = (QUEUE_HEAD + 1) % process::MAX_PROCESSES as usize;
    QUEUE_TAIL = (QUEUE_TAIL + 1) % process::MAX_PROCESSES as usize;
    ...
    if let Some(cur) = (CURRENT_SCHEDULED_PID as usize).checked_sub(1).and_then(|idx| process::PROCESS_TABLE.get_mut(idx)) {
        cur.state = process::PROC_RUNNING;
    }
}
