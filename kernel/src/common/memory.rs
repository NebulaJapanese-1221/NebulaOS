...
pub unsafe fn memory_init_physical() {
    ...
    core::ptr::write_bytes(PAGE_BITMAP, 0, PAGE_BITMAP_SIZE * 4);
...
pub unsafe fn alloc_page() -> *mut u8 {
    for i in 0..PAGE_BITMAP_SIZE {
        if *PAGE_BITMAP.add(i) != 0xFFFFFFFF {
            for j in 0..32 {
                if (*PAGE_BITMAP.add(i) & (1 << j)) == 0 {
                    let page_num = i * 32 + j;
                    *PAGE_BITMAP.add(i) |= 1 << j;
...
pub unsafe fn page_fault_handler(regs: *mut crate::common::idt::Registers) {
    crate::common::nebula::kernel_panic("Page fault");
}
